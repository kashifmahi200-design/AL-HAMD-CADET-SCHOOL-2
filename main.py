...
def init_db():
    ...
    # --- FIX: Add status column to existing tables if missing ---
    try:
    ...
    if not cursor.fetchone():
        ...
        cursor.execute("INSERT INTO students (name, contact, class_name, address, gender, username, password, father_name, cnic, dob, is_deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)",
                       ('Ali Raza', '0300-1234567', 'Class 5', 'Lahore', 'Male', sample_bform, 'student123', 'Ahmed Raza', '12345-1234567-8', '2010-01-01'))
        ...
        for i in range(2, 6):
            ...
            cursor.execute("INSERT INTO students (name, contact, class_name, address, gender, username, password, father_name, cnic, dob, is_deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)",
                           (f'Student {i}', f'0300-00000{i}', 'Class 5', 'Lahore', 'Male', bform, 'student123', f'Father {i}', bform, '2010-01-01'))
            cursor.execute("INSERT INTO fee_records (student_name, student_username, class_name, total_fee, paid_amount, remaining, status, month_year) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        ...
        cursor.execute("INSERT INTO students (name, contact, class_name, address, gender, username, password, father_name, cnic, dob, is_deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)",
                       ('Fatima', '0300-8888888', 'Class 8', 'Karachi', 'Female', bform8, 'student123', 'Ali', '87654-9876543-1', '2012-05-10'))
        ...
        cursor.execute("INSERT INTO students (name, contact, class_name, address, gender, username, password, father_name, cnic, dob, is_deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)",
                       ('Bilal', '0300-9999999', 'Class 6', 'Islamabad', 'Male', bform6, 'student123', 'Khalid', '11111-2222222-3', '2011-03-15'))
...
def get_class_color(class_name):
    # simple color mapping for visual variety
    colors = [(0.05,0.28,0.63,1), (0.2,0.6,0.2,1), (0.8,0.5,0.1,1), (0.5,0.2,0.7,1)]
...
class LoginScreen(Screen):
    def process_login(self):
        ...
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute("SELECT role, status FROM users WHERE username=? AND password=?", (u, p))
        user = c.fetchone()
        conn.close()
        if user:
            role, status = user
            if status == 'Approved':
                ...
                if role == 'Student':
                    ...
                    cur = conn2.cursor()
                    cur.execute("SELECT class_name, gender FROM users WHERE username=?", (u,))
                    row = cur.fetchone()
                    app.student_class = row[0] if row else None
                    app.student_gender = row[1] if row else None
                    conn2.close()
...
class DashboardScreen(Screen):
    ...
    def load_user_role(self, role):
        self.user_role = role
        if is_admin_or_principal_or_head(role):
            for card_id in ['pwd_btn', 'principal_card', 'heads_card', 'teachers_card', 'students_card', 'fee_card', 'class_card', 'date_sheet_card', 'timetable_card', 'ai_card', 'settings_card', 'my_result_access_card']:
                if hasattr(self.ids, card_id):
                    self.ids[card_id].opacity = 1
                    self.ids[card_id].disabled = False
        elif is_teacher(role):
            for card_id in ['principal_card', 'heads_card', 'teachers_card', 'students_card', 'fee_card', 'class_card', 'date_sheet_card', 'timetable_card', 'ai_card', 'settings_card', 'my_result_access_card']:
                if hasattr(self.ids, card_id):
                    self.ids[card_id].opacity = 1
                    self.ids[card_id].disabled = False
            if hasattr(self.ids, 'pwd_btn'):
                self.ids.pwd_btn.opacity = 0
                self.ids.pwd_btn.disabled = True
        elif is_student(role):
            for card_id in ['pwd_btn', 'principal_card', 'heads_card', 'teachers_card', 'class_card', 'timetable_card', 'ai_card', 'settings_card', 'my_result_access_card']:
                if hasattr(self.ids, card_id):
                    self.ids[card_id].opacity = 0
                    self.ids[card_id].disabled = True
            if hasattr(self.ids, 'students_card'):
                self.ids.students_card.opacity = 1
                self.ids.students_card.disabled = False
            if hasattr(self.ids, 'fee_card'):
                self.ids.fee_card.opacity = 1
                self.ids.fee_card.disabled = False
            if hasattr(self.ids, 'date_sheet_card'):
                self.ids.date_sheet_card.opacity = 1
                self.ids.date_sheet_card.disabled = False
    def logout(self):
        self.manager.current = 'login'
...
class PrincipalPanelScreen(Screen):
    ...
    def load_notifications(self):
        ...
        self.ids.notification_card.children[0].text = f"STUDENT CHANGE REQUESTS ({len(logs)})"
    ...
    def load_heads_list(self):
        ...
        if not heads:
            ...
            for idx, h in enumerate(heads, 1):
                ...
                card = MDCard(orientation='vertical', padding=15, spacing=0, size_hint_y=None, height=420, radius=15, elevation=4, md_bg_color=(1,1,1,1))
                name_box = MDBoxLayout(size_hint_y=None, height=45, padding=[10,0,0,0])
                name_box.add_widget(MDLabel(text=f"{idx}. {name}", bold=True, theme_text_color="Custom", text_color=color, font_style="H6"))
    ...
    def load_teachers_list(self):
        ...
        if not teachers:
            ...
            for idx, t in enumerate(teachers, 1):
                ...
                if can_edit:
                    edit_btn = MDRaisedButton(text="EDIT", md_bg_color=(0.2,0.7,0.3,1), size_hint_x=0.33,
                    ...
                    btn_box.add_widget(edit_btn)
                    delete_btn = MDRaisedButton(text="DELETE", md_bg_color=(0.8,0.2,0.2,1), size_hint_x=0.33,
                                                on_release=lambda x, username=username: self.delete_teacher(username))
                    btn_box.add_widget(delete_btn)
                atten_btn = MDRaisedButton(text="Atten", md_bg_color=(0.05,0.28,0.63,1), size_hint_x=0.34,
...
class StudentsPanelScreen(Screen):
    ...
    def load_students_list(self):
        ...
        c = conn.cursor()
        if is_student(app.role):
            c.execute("SELECT s.id, s.name, s.contact, s.class_name, s.address, s.gender, s.username, u.status FROM students s JOIN users u ON s.username = u.username WHERE s.username=? AND s.is_deleted=0 ORDER BY s.class_name, s.name", (app.username,))
        else:
        ...
        if not students:
            ...
            for idx, s in enumerate(students, 1):
                ...
                name_box = MDBoxLayout(size_hint_y=None, height=45, padding=[10,0,0,0])
                name_label = MDLabel(text=f"{idx}. {name}", bold=True, theme_text_color="Custom", text_color=color, font_style="H6")
                name_box.add_widget(name_label)
                card.add_widget(name_box)
                card.add_widget(MDBoxLayout(size_hint_y=None, height=2, md_bg_color=(0.85,0.85,0.85,1)))
    ...
    def open_edit_student_dialog(self, sid, name, gender, contact, address, class_name, username):
        app = MDApp.get_running_app()
        if not is_admin_or_principal_or_head(app.role) and not (is_student(app.role) and sid == self.get_own_student_id()):
            self.show_dialog("Access Denied", "You can only edit your own profile.")
            return
        scroll = ScrollView(size_hint=(1, None), height=Window.height*0.8)
        ...
        dlg = MDDialog(title="", type="custom", content_cls=scroll,
                       buttons=[MDFlatButton(text="CANCEL", on_release=lambda x: dlg.dismiss()),
                                MDRaisedButton(text="UPDATE", md_bg_color=(0.2,0.7,0.3,1),
                                               on_release=lambda x: self.update_student(sid, name_f.text, edit_gender["value"], contact_f.text, address_f.text, class_f.text, dlg))])
        dlg.open()
    def update_student(self, sid, name, gender, contact, address, class_name, dlg):
        if not all([name.strip(), contact.strip(), address.strip(), class_name.strip()]):
            self.show_dialog("Error", "All fields required!")
            return
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute("UPDATE students SET name=?, gender=?, contact=?, address=?, class_name=? WHERE id=?",
                  (name.strip(), gender, contact.strip(), address.strip(), class_name.strip(), sid))
        c.execute("UPDATE users SET name=?, class_name=?, gender=?, contact=?, address=? WHERE username=(SELECT username FROM students WHERE id=?)",
                  (name.strip(), class_name.strip(), gender, contact.strip(), address.strip(), sid))
        conn.commit()
        conn.close()
        dlg.dismiss()
        self.load_students_list()
    ...
    def open_add_student_dialog(self):
        ...
        dlg = MDDialog(title="", type="custom", content_cls=scroll,
                       buttons=[MDFlatButton(text="CANCEL", on_release=lambda x: dlg.dismiss()),
                                MDRaisedButton(text="ADD", md_bg_color=(0.05,0.28,0.63,1),
                                               on_release=lambda x: self.add_student(name_f.text, contact_f.text, class_f.text, address_f.text, self.selected_gender, bform_f.text, pwd_f.text, dlg))])
        dlg.open()
    def add_student(self, name, contact, class_name, address, gender, bform, pwd, dlg):
        ...
        try:
            ...
            c.execute("INSERT INTO students (name, contact, class_name, address, gender, username, password, father_name, cnic, dob, is_deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)",
                      (name.strip(), contact.strip(), class_name.strip(), address.strip(), gender, bform, pwd, '', '', ''))
...
class ClassPanelScreen(Screen):
    ...
    def load_classes(self):
        ...
        for class_name in classes:
            ...
            conn.close()
            color = get_class_color(class_name)
            card = MDCard(orientation='vertical', size_hint_x=1, size_hint_y=None, height=dp(140),
                          radius=14, elevation=3, md_bg_color=(1,1,1,1), padding=12, spacing=8)
            label = MDLabel(text=f"[b]{class_name}[/b]\nStudents: {count}  |  Teacher: {teacher_name}", 
                            markup=True, halign='center', valign='middle', size_hint_y=0.5,
                            theme_text_color="Custom", text_color=(0.05,0.28,0.63,1))
            label.bind(size=lambda l, s: setattr(l, 'text_size', (s[0], None)))
            btn_layout = MDBoxLayout(orientation='horizontal', spacing=10, size_hint_y=0.5, height=dp(45))
            students_btn = MDRaisedButton(text="Students", md_bg_color=(0.05,0.28,0.63,1), size_hint_x=0.33,
                                          on_release=lambda x, cn=class_name: self.edit_class_students(cn))
            results_btn = MDRaisedButton(text="Results", md_bg_color=(0.05,0.28,0.63,1), size_hint_x=0.33,
...
class StudentEditScreen(Screen):
    ...
    def load_students(self):
        ...
        if not students:
            ...
            for student in students:
                ...
                dob_field = MDTextField(text=dob if dob else "", hint_text="Date of Birth (YYYY-MM-DD)", mode="rectangle", size_hint_y=None, height=60)
    ...
    def generate_pdf_report(self):
        ...
        if not is_admin_or_principal_or_head(app.role):
            ...
            return
        if not REPORTLAB_AVAILABLE:
            self.show_dialog("Error", "ReportLab is not installed. Please install it using: pip install reportlab")
            return
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
...
class FeeLedgerScreen(Screen):
    ...
    def load_classes(self):
        ...
        if is_student(app.role):
            ...
            card = MDCard(orientation='vertical', size_hint_x=1, size_hint_y=None, height=dp(120),
                          radius=14, elevation=3, md_bg_color=(1,1,1,1), padding=0)
            layout = RelativeLayout(size_hint=(1,1))
            label_text = f"[b]{student_class}[/b]\nStudents: {count}\nMonth: {self.current_month}"
            label = MDLabel(text=label_text, markup=True, halign='center', valign='center',
                            theme_text_color="Custom", text_color=(0.05,0.28,0.63,1),
                            font_style="Body1", bold=False,
            ...
            grid = GridLayout(cols=2, spacing=dp(14), size_hint_y=None)
            ...
            card = MDCard(orientation='horizontal', size_hint_x=1, size_hint_y=None, height=dp(120),
                          radius=14, elevation=3, md_bg_color=(1,1,1,1), padding=0)
            layout = RelativeLayout(size_hint=(1,1))
            label_text = f"[b]Class 10[/b]\nStudents: {count}  |  Collection: Rs. {total:,}\nMonth: {self.current_month}"
            label = MDLabel(text=label_text, markup=True, halign='center', valign='center',
                            theme_text_color="Custom", text_color=(0.05,0.28,0.63,1),
                            font_style="Body1", bold=False,
                            size_hint=(1,1), pos_hint={'center_x':0.5,'center_y':0.5})
...
class TimetableScreen(Screen):
    ...
    def load_cards(self):
        ...
        for class_name in other_classes:
            ...
            for entry in data:
                if entry.get("time") and entry.get("subject"):
                    ...
                    break
            conn.close()
            card = MDCard(orientation='vertical', size_hint_x=1, size_hint_y=None, height=dp(150),
                          radius=14, elevation=4, md_bg_color=(1,1,1,1), padding=8, spacing=6)
            card.add_widget(MDLabel(text=class_name, bold=True, font_style="H6", halign="center", size_hint_y=None, height=dp(30), theme_text_color="Custom", text_color=(0.05,0.28,0.63,1)))
            card.add_widget(MDLabel(text=preview, font_style="Caption", theme_text_color="Secondary",
                                    halign="center", size_hint_y=None, height=dp(40)))
            btn_layout = MDBoxLayout(orientation='horizontal', spacing=10, size_hint_y=None, height=dp(40))
        ...
        card.add_widget(MDLabel(text=preview, font_style="Caption", theme_text_color="Secondary",
                                halign="center", size_hint_y=None, height=dp(40)))
        btn_layout = MDBoxLayout(orientation='horizontal', spacing=10, size_hint=(None,None), size=(dp(250), dp(40)), pos_hint={'center_x':0.5})
        view_btn = MDRaisedButton(text="VIEW", md_bg_color=(0.05,0.28,0.63,1), size_hint_x=0.5)
        edit_btn = MDRaisedButton(text="EDIT", md_bg_color=(0.05,0.28,0.63,1), size_hint_x=0.5)
        view_btn.bind(on_release=lambda x, cn=class_name: self.view_timetable(cn))
        edit_btn.bind(on_release=lambda x, cn=class_name: self.edit_timetable(cn))
        btn_layout.add_widget(view_btn)
        btn_layout.add_widget(edit_btn)
        card.add_widget(btn_layout)
        container.add_widget(card)
...
class DateSheetScreen(Screen):
    ...
    def load_cards(self):
        ...
        view_btn = MDRaisedButton(text="VIEW", md_bg_color=(0.05,0.28,0.63,1), size_hint_x=0.5)
        edit_btn = MDRaisedButton(text="EDIT", md_bg_color=(0.05,0.28,0.63,1), size_hint_x=0.5)
        view_btn.bind(on_release=lambda x, cn=class_name: self.view_datesheet(cn))
        edit_btn.bind(on_release=lambda x, cn=class_name: self.edit_datesheet(cn))
        btn_layout.add_widget(view_btn)
        btn_layout.add_widget(edit_btn)
        card.add_widget(btn_layout)
        container.add_widget(card)
